...
- ❌ 分享 / 社交
- ❌ 导出笔记
- ❌ EPUB / Mobi / 网页文章（只 PDF）—— **下一轮主攻 EPUB**，详见
  [`docs/epub-support.md`](./epub-support.md)
...
- 2026-05-04 **Landing 包瘦身**：用户反馈 Chrome 第一次打开有点慢。诊断到初始 JS 850 KB（`supabase-js` 全部 SDK 进了每个页面的初始 bundle，仅仅是 Nav 用 `createClient` 做 Sign Out + UploadDropzone 用 `uploadToSignedUrl`）。两层修：(a) `components/LazyUploadDropzone.tsx` 用 `next/dynamic({ ssr: false })` 把 dropzone 推到 lazy chunk；(b) `Nav.tsx` 把 `createClient` 改成 SignOut handler 内 inline 动态 import。结果初始 JS **850 → 627 KB**（-223 KB / -26%）。Supabase 那 200 KB chunk 只在用户真点 Sign Out 或拖文件时才下载。
- 2026-05-06 **Landing 静态预渲染**——上一波瘦身后体感还是慢。再深挖：response headers 是 `cache-control: private, no-cache, no-store` + `x-vercel-cache: MISS`，意思是**每次访问都触发 Vercel function 调用**。Cold function start 单次 ~800ms-1s。Root cause: `app/layout.tsx` 是 async 调了 `supabase.auth.getUser()`，读 cookies 让整个 root layout 变 dynamic，所有路由（含 landing）都被强制 SSR。修：layout 改 sync 不再做 auth fetch；Nav 改成 client-side `useEffect` 调一个新加的 `/api/me` 轻量 endpoint 拿 auth state。Build 输出：`/` 从 `ƒ` (dynamic) 变 `○` (prerendered)。TTFB **800ms cold / 220ms warm → 120-155ms 全球一致**（CDN HIT）。Trade-off：Nav 的"Library / Sign out"按钮 hydrate 后 ~100-200ms 才出现，预留了占位无 layout shift。
- 2026-05-08 **Failed to save chapters bug 修复**——真用户传某本 PDF 撞到 Postgres `22P05:   cannot be converted to text`。PDF 字体里嵌入子集（中文书 / DRM 版 / OCR'd 扫描版常见）的 glyph 找不到 Unicode 映射时，unpdf 会塞 ` ` 占位，但 Postgres `text` / `jsonb` 列不接受 NUL byte。三层防御：`parser.ts` 加 `stripNul()` helper 在源头清；`outline.ts` 章节内容 + TOC 标题清；`finalize/route.ts` insert 前再清 + 1MB content cap。NUL byte 隐藏看不见，删掉零损失。
- 2026-05-09 **首批用户接入 + cold reach-out 工具链开源**——Twitter 推产品后 24h 内来了 33 个真用户、传了 38 本书，几乎覆盖中文书 / 英文书 / 技术 / 哲学 / 投资全谱。建了个 indie founder 直发邮件的工具链：`scripts/draft-cold-emails.mjs` 拿 Auth 用户列表 + diff `marketing/sent-emails.json` 跳过列表 → 套模板生成今天的 batch txt；`scripts/send-cold-emails.mjs` 用 Gmail SMTP 一封封发（5s 间隔，每发一封写 registry，重跑幂等）。两套中文模板（uploaded vs signed-up-no-upload）。当天给 33 个用户发完。`marketing/templates/` + `scripts/` 全部 commit 公开（其他 indie founder 可以直接 fork 用），`marketing/sent-emails.json` 等含 PII 的文件用 selective gitignore 留在本地。
- 2026-05-10 **影子图书馆水印 cover 过滤 + scan-only PDF 友好 422**——首批用户 Frankie 反馈 Brief 出来全是 "Anna 的档案 / 文件名 _xxxx.zip" 这种乱码。Root cause：他上传的是从影子图书馆（Anna's Archive / DuXiu / 读秀）下载的 PDF，最前面藏一页"水印 cover"装得像第一章（archive 元信息、base64 ID、MD5/SHA-256 哈希），而正文页是图片扫描没文字层 —— 系统读到的"内容"只有水印那一页，AI 自然只能基于水印写 Brief。新增 `looksLikeShadowLibraryWatermark(content)` 检测器：长度 < 3000 字 + 命中 6 个指纹之一（Anna's Archive / DuXiu|读秀 / Z-Library / LibGen / Sci-Hub / `_<digits>.zip|pdf|epub` / 长 base64 / SHA-256 / MD5）→ 自动过滤。过滤完一章不剩则返回友好 422 提示用户换带文字层的 PDF。同日跟 Frankie 邮件回复确认 fix 上线 + 50MB 是测试版限制 + EPUB 是下一轮主攻方向。

**下一步**：**EPUB 格式支持**——首批用户里多人主动问"能不能传 EPUB"（程序员书、中文小说很多原生就是 EPUB）。EPUB 比 PDF 友好得多（结构化章节 + 干净的 title/author 元数据 + 不踩 PDF 那几个坑），是回报最高的下一步。详细 plan 见 [`docs/epub-support.md`](./epub-support.md)，预估 2–3 个工作日。Sentry / Posthog（§12.C 朋友测前的硬化收尾）排在 EPUB 之后。
