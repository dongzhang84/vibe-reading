...
一个 4 屏的工作流（Book Home 内嵌 Orientation ritual）：
...
2. **Book Home** — 进入这本书的主页：
   - **首先 Orient yourself**（4 个用户自答问题，**AI 一句话不写**）：
     1. 这本书写的是什么样的主题？
     2. 作者是谁，什么背景？
     3. 这本书是写给谁的？
     4. 你希望从这本书 take away 什么？
   - 答完 take-away 才**解锁**下方的 Ask 区
   - 然后展示 TOC + 问题输入框 + 3 AI 推荐问题（一键提交）+ 历史问过的问题列表（可重新打开任一）
   - take-away 答案在 Book Home 顶部 pinned 显示，并作为 context 注入下游 relevance AI 提升匹配精度
...
**为什么 Orientation 嵌进 Book Home 而不是单独一屏**：少一次跳转，答完之后 4 段答案不浪费（持续作为 context 跟着用户每次提问），Book Home 同时承载"调钩子 (orient)"和"提问 (ask)"两件事，符合"一个屏一个心智状态"的简洁原则。
...
> **Orientation 是 Rule 1 的纯粹形态**：Book Home 顶部 4 个问题（主题 / 作者 / 目标读者 / take-away）**全部用户自答**，AI 一句话不写。即便是元信息层面的"这本书是什么"也不替读者压缩 —— 让读者自己翻前言、看封底、调动已有钩子。take-away 答完才解锁 Ask 区。这一步把"compression must happen in your brain"从 spec 立场推到 UI 强制 ritual。
...
- 2026-04-29 v2.1 Book Home 内嵌 **Orientation ritual**：4 个用户自答问题（主题 / 作者 / 目标读者 / take-away），take-away 答完才解锁 Ask 区，并作为 context 注入下游 relevance AI
