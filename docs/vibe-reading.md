...
- 2026-04-30 Orientation 简化为**静态展示**：去掉 textarea / DB 列 / Ask 区 gate / relevance 注入。结论："AI 不替用户压缩"不等于"用户必须打字"——4 个问题本身就是钩子，被问到那一刻就触发了 orient。同日修两个真实使用 bug：(a) PDF 没 Title metadata 时回退到文件名，避免书名变 "Untitled"；(b) Part-Chapter 结构的书（如 _Beyond Vibe Coding_）原本只按 level-1 切，导致 Part I/II/III 被当成单一章节喂给 relevance AI，加 chapter-level picker + front-matter 过滤后修复
