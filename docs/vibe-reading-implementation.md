...
vibe-reading 实际执行（2026-05-02）：买了 `vibe-reading.dev`（带连字符跟整个 repo / Vercel project / 文档命名一致；`vibereading.dev` 被划进 Google Registry premium 拿不到合理价）。Namecheap Advanced DNS 删 parking 记录后加 Vercel 给的 A + CNAME。Supabase Auth Site URL 改为 `https://vibe-reading.dev`，Redirect URLs 加 `/auth/callback` 的 apex + www 两条。代码 0 改动（`NEXT_PUBLIC_APP_URL` 当前没被代码引用，metadata 也无 `metadataBase`）；只动 4 个文档里的 live URL 文案。原 `vibe-reading-iota.vercel.app` 保持可访问。

> **当时遇到的两个非显然的坑**（值得记给 indie-playbook §12.B 增强）：
>
> **(1) Vercel "Add Domain" 默认把 www 当主域**：在 Vercel 项目里 Add `vibe-reading.dev` 时即便填的是不带 www 的 apex，Vercel 也会自动把 `www.vibe-reading.dev` 设成 Production 主，apex 307 跳到 www。需要手动到 Settings → Domains 编辑，把 apex 改成 "Connect to environment: Production"、把 www 改成 "Redirect to Another Domain → vibe-reading.dev"。这一步 Vercel UI 很容易错过。
>
> **(2) Vercel dashboard 的 deployment 略缩图卡在第一次截图（403）**：Vercel 的 thumbnail 服务似乎只在第一次部署时抓一张图，之后即便 redeploy / 改了 Deployment Protection 都不重抓——dashboard 上看到的 403 截图永远不变。`curl` 实测 apex 200、www 307 都正常。**纯装饰性 bug**，不影响用户、SEO、产品任何东西，只有自己登 Vercel dashboard 时才看到。可以忽略。
